...
def parse_meeting_agenda(scraper: LACouncilScraper, meeting: Dict) -> bool:
    ...
    if not portal_url:
        print(f"⚠️  No HTML Agenda available for this meeting")
        return False
    ...
    try:
        ...
        print(f"❌ Could not extract template ID from URL")
        return False

    # Download HTML
    print(f"📥 Downloading agenda HTML...")
    html_content = scraper.fetch_document_content(portal_url)

    if not html_content:
        print(f"❌ Failed to download agenda")
        return False

    print(f"✅ Downloaded {len(html_content):,} bytes")

    # Parse HTML
    print(f"🔍 Parsing agenda structure...")
    parser = AgendaParser(html_content, meeting_id, template_id)
    agenda = parser.parse()

    print(f"✅ Parsed {agenda['total_items']} items in {agenda['total_sections']} sections")
...
def main():
    ...
    print(f"⏭️  Skipped (already parsed): {skip_count}")
    print(f"⚠️  Failed/No agenda: {len(meetings) - success_count - skip_count}")
    print(f"📁 Agendas saved to: data/agendas/")
    print("=" * 60)
