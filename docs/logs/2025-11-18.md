...
## 2025-11-18 - Add PDF processing analysis and debugging utilities

**Commit Message:** Add PDF processing analysis and debugging utilities

**Files Changed:**
- `analyze_failures.py` - Log parser for categorizing PDF processing failures (new file)
- `analyze_large_pdfs.py` - Analyzer for large PDF documents (new file)
- `analyze_retry_failures.py` - Retry log analyzer (new file)
- `check_public_comments.py` - Public comment document checker for council file 25-1037 (new file)
- `find_appeal_response.py` - Appeal response document finder (new file)
- `find_failed_pdfs.py` - Failed document tracker with PDF URL generator (new file)
- `show_massive_files.py` - Massive file analyzer (new file)
- `pdf_processing.log` - Initial processing run log (new file)
- `pdf_processing_retry.log` - Retry processing run log (new file)

**Summary:**
Added suite of Python utility scripts for analyzing and debugging PDF processing runs. Scripts parse processing logs to categorize failures (page limits, token limits, request size), identify problematic documents, and generate direct PDF URLs for manual inspection. Includes specialized tools for checking public comment documents and tracking massive files that exceed processing limits.

Logs capture complete processing runs showing 93.7% success rate with detailed error categorization for the 22 unprocessed documents.
